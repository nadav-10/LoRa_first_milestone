...
cached_user_details = {}
cached_details_lock = threading.Lock()
...
class ServerBE:
    ...
    def handle_alive_calls(data : dict) -> list[dict]:
        ...
        encrypted_info = base64.b64decode(data.get("info_b64"))
        with users_lock:
...
def alive():
    ...
    if not data or 'key_b64' not in data or "info_b64" not in data:
        return jsonify({
            "status": "error",
